async function computeAsync(uri, dtoIn, session) {
  ...
  const computationRequest = await this._checkAndGetComputationRequest(
    awid,
    dtoIn,
    uuAppErrorMap
  );
  ...
  const beAfrrComputationType = this._getBeAfrrComputationType(
    computationRequest.viewFullCode,
    uuAppErrorMap
  );

  // 4. Process computation request.
  await this._processComputationRequest(
    awid,
    dtoIn,
    beAfrrComputationType,
    computationRequest,
    uri,
    session,
    uuAppErrorMap
