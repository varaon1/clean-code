async function create(uri, dtoIn, session, uuAppErrorMap = {}) {
  ...
  if (!uuSprintMan) {
    // A1
    throw new Errors.Create.UuSprintManDoesNotExist(
      { uuAppErrorMap },
      { awid }
    );
  }
  ...
  if (
    uuSprintMan.state === States.closed ||
    uuSprintMan.state === States.passive
  ) {
    // A2
    throw new Errors.Create.UuSprintManIsNotInProperState(
      { uuAppErrorMap },
      { uuSprintMan: { state: uuSprintMan.state } }
    );
  }
  ...
  const anchoredTimezone = uuSprintMan.anchoredTimezone
    ? uuSprintMan.anchoredTimezone
    : DefaultAnchoredTimezone;
  const endOfDay = DateTimeHelper.getEndOfDay(now, anchoredTimezone);
  if (dtoIn.planStartDate && dtoIn.planEndDate) {
    // HDS 3.1
    // HDS 3.1.1
    dtoIn.planStartDate = DateTimeHelper.getStartOfDay(
      dtoIn.planStartDate,
      anchoredTimezone
    );
    // Get start of the next day of planEndDate
    dtoIn.planEndDate = DateTimeHelper.getStartOfDay(
      DateTimeHelper.getNextDay(dtoIn.planEndDate, anchoredTimezone),
      anchoredTimezone
    );
    // HDS 3.1.2
    if (dtoIn.planStartDate >= dtoIn.planEndDate) {
      // A6
      throw new Errors.Create.EndIsBeforeStart(
        { uuAppErrorMap },
        { planStartDate: dtoIn.planStartDate, planEndDate: dtoIn.planEndDate }
      );
    }
    ...
    let planEndDate = DateTimeHelper.getNextDay(
      dtoIn.planStartDate,
      anchoredTimezone,
      uuSprintMan.defaultSprintLength
    );
    dtoIn.planEndDate = DateTimeHelper.getStartOfDay(
      planEndDate,
      anchoredTimezone
    ...
    if (!previousSprint) {
      // A7
      throw new Errors.Create.PreviousSprintDoesNotExist(
        { uuAppErrorMap },
        { previous: dtoIn.previous }
      );
    }
  ...
  // HDS 4
  const overlaps = await this.dao.findDateOverlaps(
    awid,
    dtoIn.planStartDate,
    dtoIn.planEndDate
  );
  if (overlaps.itemList.length > 0) {
    ...
    throw new Errors.Create.ExistsOverlappingSprints(
      { uuAppErrorMap },
      { sprints }
    );
  }
  ...
  // HDS 5, A9
  if (!previousSprint)
    previousSprint = await this.dao.findPreviousByStartDate(
      awid,
      dtoIn.planStartDate
    );
  ...
  if (!dtoIn.state) {
    ...
    //HDS 6.2
      ...
      if (currentSprint)
        throw new Errors.Create.CurrentSprintAlreadyExists(
          { uuAppErrorMap },
  ...
  if (previousSprint) {
    ...
    // HDS 9.2
    const nextSprint = await this.dao.findNextByEndDate(
      awid,
      dtoIn.planEndDate
    );
  ...
  // HDS 11
  try {
    await SprintOrderAbl.insertSprintToOrderInternal(
      awid,
      sprint.id,
      orderOptions
    );
    ...
    try {
      ...
      // A13
      throw new Errors.Create.SprintDaoCreateRollbackFailed(
        { uuAppErrorMap },
        e2
      );
    }
    ...
    // A12.2
    throw new Errors.Create.InsertSprintToSprintOrderFailed(
      { uuAppErrorMap },
      e
    );
  }
  ...
  if (uuSprintMan.authorizationStrategy === AuthorizationStrategies.artifact) {
    // HDS 12.1
    const sysIdentitySession =
      await AuthenticationService.authenticateSystemIdentity();
    const appTokenHandler = new AppTokenHandler(uri, uuSprintMan.btBaseUri);
    const uuFolder = new UuFolderAPI(appTokenHandler);
    const mainFolderAttributes = UriBuilder.parse(
      uuSprintMan.sprintsFolderUri
    ).getParameters();

    // HDS 12.2
    const uuSprintManVersionCode = Folders.getSprintManMainCode(uuSprintMan);
    const targetYearFolderAttributes =
      Folders.baseFolders.getSprintsYearAttributes(
        uuSprintManVersionCode,
        dtoIn.planStartDate,
        awid
      );

    let targetYearFolder = await uuFolder.getIfExists(
      { code: targetYearFolderAttributes.code },
      session
    );
    if (!targetYearFolder) {
      //A14
      let sprintFolderLocationAttributes = {
        ...
        explicitPermissionList: uuFolder.prepareExplicitPermissions(
          uuSprintMan.folderExplicitRights
        ),
      };
      try {
        targetYearFolder = await uuFolder.create(
          sprintFolderLocationAttributes,
          session
        );
      } catch (e) {
        //A15
        //A15.1 and inner A16 and A17
        await RollbacksHelper.rollbacksSprintCreate(
          awid,
          sprint.id,
          Errors.Create,
          uuAppErrorMap
    ...
    try {
      sprintFolder = await uuFolder.create(
        sprintFolderLocationAttributes,
        session
      );
    } catch (e) {
    ...
    try {
      sprintLogFolder = await uuFolder.create(
        sprintLogFolderLocationAttributes,
        session
      );
    } catch (e) {
      // A21
      ...
      // A21.2 and inner A24
      await RollbacksHelper.rollbackDeleteFolder(
        uuFolder,
        sprint.id,
        session,
        Errors.Create.DeleteFolderFailed,
        uuAppErrorMap
      );
      // A21.3
      throw new Errors.Create.CreateFolderFailed({ uuAppErrorMap }, e);
    ...
    if (aarDtoIn.artifactB) {
      ...
      try {
        uuArtifactRelation = await uuObcApi.createAar(
          aarDtoIn,
          sysIdentitySession
        );
      } catch (e) {
  ...
  // HDS 13
  const adjacentSprints = await SprintOrderAbl.getAdjacentSprintsInternal(
    awid,
    sprint.id
  );
